...
<body><h2>multi-gf-mode</h2><p>Generated: ${now}</p><img src="${qrDataUrl}" /><p>Scan with WhatsApp → Linked Devices → Link a Device</p></body></html>`;
    ...
    client.on("ready", async () => {
        ...
        if (allowed.length > 0) {
            ...
            console.error("\n⚠️  No ALLOWED_CONTACTS set in .env!");
            console.error("   The bot won't reply to anyone until you add contacts.");
            console.error("   Edit .env → ALLOWED_CONTACTS=919876543210,91XXXXXXXXXX");
            console.error("   Or send /allow in a chat to add contacts on the fly.\n");
        }
    });
    ...
    client.on("message", async message => {
        ...
        // --- Owner-only commands (only you can use these) ---
        if (message.body.startsWith("/") && message.fromMe) {
            ...
            if (message.body === "/allow") {
                ...
                const phone = await resolvePhone(contactId);
        ...
        // Ignore all other fromMe messages and commands from others
        ...
        // --- Allowlist check ---
        if (allowed.length === 0) {
            return; // No contacts configured, don't reply to anyone
        }
